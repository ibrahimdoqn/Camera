...
- **Gizle/göster akışı çökmez**: önceki sürümde `Izgaradan gizle`'ye basıldığında uygulama bazen ölüyordu. İki ayrı kök neden vardı: (1) `tile.deleteLater()`'dan sonra arka plandaki RTSP iş parçacığı hâlâ `frame_ready` / `status_changed` sinyali kuyruğa atıyordu — artık `stop()` worker'a `_running=False` demeden önce sinyalleri **disconnect** ediyor, böylece worker birkaç saniye daha çalışsa bile yayınladığı sinyallerin gidecek bir alıcısı kalmıyor; (2) tile'ı yıkma işi (`grid.set_cameras` → libVLC `stop`, RTSP capture serbest bırakma, `deleteLater` zinciri) `QPushButton` *click* dispatch'i hâlâ stack'teyken çalışıyordu. Windows'ta Qt'nin button state machine'i ile bu yıkımın iç içe geçmesi süreç çökmesine sebep oluyordu. Artık `_on_visibility_toggled` handler'ı yapılandırmayı / durum çubuğunu güncelleyip kontrolü hemen iade ediyor; ızgara yeniden inşası sürükle-bırak için kullanılan **`QTimer.singleShot(0, …)`** pattern'iyle bir sonraki event loop turuna ertelenir, böylece tile yıkımı click handler'ı tamamen unwind ettikten sonra çalışır. Buna ek olarak libVLC nesneleri için artık `release()` çağrılmıyor (bu çağrı bazı sürümlerde yerel bir crash tetikleyebiliyordu); sadece `stop()` çağrılıp Python referansı bırakılıyor
