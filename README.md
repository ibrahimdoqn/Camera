...
- **Ayarlar penceresi**: hedef FPS, sütun sayısı, yeniden bağlanma süresi, durum rozetini gösterme, **log kayıtları**, **log seviyesi**, **MAC tabanlı IP yeniden keşif** — değişiklikler **canlı** uygulanır, FPS değişimi artık uygulamayı kilitlemez. Donanım hızlandırma seçilebilir bir ayar değildir; libVLC uygun GPU decoder'ını kendisi seçer
- **libVLC dekoder + otomatik donanım hızlandırma**: uygulama tüm RTSP akışlarını **libVLC** üzerinden çözer. VLC, platformdaki mevcut donanım decoder'ını **kendi seçer** — Windows'ta DXVA2 / D3D11, Intel iGPU'da Quick Sync, NVIDIA'da NVDEC. Hiçbirisi çalışmazsa sessizce yazılım decoder'a düşer. Kullanıcının seçim yapması gerekmez; ayarlarda HW hızlandırma seçeneği bulunmaz. **Kareler GPU'da çözüldükten sonra** doğrudan tile'ın paint pipeline'ına akar — böylece cursor-merkezli zoom, pan, overlay rozeti ve tam ekran özelliği hepsi çalışmaya devam ederken CPU yükü belirgin biçimde düşer
- Otomatik **yeniden bağlanma** ve canlı **bağlantı durumu rozeti** (connecting / online / offline). VLCFrameWorker iki katmanlı bir **gözcü (watchdog)** çalıştırır: (1) VLC'ye `--network-caching=300 --live-caching=300` verilir ve `--rtsp-tcp` ile UDP paket kaybı yaşanmadığından emin olunur; (2) ilk kare **8 saniye** içinde gelmezse veya son başarılı kareden bu yana **8 saniye** geçmişse bağlantı zorla kapatılıp `reconnect_delay` sonrası yeniden açılır. Sonuç olarak çevrim dışı kalan her akış kendiliğinden offline rozetine düşer ve aynı yeniden bağlanma döngüsünü kullanır
...
Her **görünür** kamera kendi `QThread`'inde çalışır. Sol menüdeki göz simgesiyle gizlenen kameralar için `CameraTile` ve `VLCFrameWorker` hiç oluşturulmaz, böylece RTSP/dekoder yükü tamamen ortadan kalkar; kullanıcı tekrar görünür yaptığında thread baştan ayağa kalkar. Video çözümlemesi tamamen **libVLC** üzerinden yürür: her worker sade bir `vlc.Instance` oluşturur ve per-media opsiyonlar ekler (`:rtsp-tcp`, `:network-caching=300`, `:live-caching=300`). Donanım hızlandırma için ayrı bir bayrak geçilmez — VLC'nin `avcodec-hw=any` varsayılanı **platformdaki tüm HW yollarını** (DXVA2 / D3D11 / NVDEC / Quick Sync / VAAPI / …) sırayla dener ve ilk başarılı olanı seçer; hiçbirisi çalışmazsa yazılım decoder'a düşer. Kareler `video_set_format_callbacks` + `video_set_callbacks` çifti üzerinden **BGRA** olarak worker'ın önceden ayırdığı ctypes buffer'ına yazılır. Worker frame'i `numpy` dizisine kopyalayıp `pyqtSignal` ile UI thread'ine iletir, `CameraTile._on_frame` bunu `QImage.Format_ARGB32` olarak sarıp `QPainter` üzerinden çizer — böylece cursor merkezli zoom, sürükleyerek pan, sağ tık zoom sıfırlama ve overlay rozeti aynen çalışır. Ses aynı libVLC üzerinden ayrı bir player ile çalınır (RTSP'yi destekler); libVLC bulunmazsa Qt `QMediaPlayer` yedek olarak denenir. Varsayılan olarak tüm sesler kapalıdır. Ses durdurulduğunda libVLC nesneleri sadece `stop()` çağrısıyla kapatılıp Python referansları düşürülür — GC daha sonra `release()`'i sessiz bir anda tamamlar (click handler stack'inde tetiklenen `release()` bazı libvlc sürümlerinde çöküyordu).

`VLCFrameWorker` bir **gözcü** mantığı çalıştırır. VLC oynatmaya başladığında her `unlock_cb`'de son kare zaman damgası yenilenir. **8 saniye** içinde ilk kare gelmezse ya da çalışan bir akış 8 saniye boyunca taze kare üretmezse VLC durdurulur, dış döngü `offline` rozetini yayar ve `reconnect_delay` sonrası yeniden bağlanır.
